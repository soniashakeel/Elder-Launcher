...
- `app_pojavlauncher` (`MainApp`) — ELDER Compose screens plus the upstream Android launch pipeline.
- `core` (`Core`) — Android library boundary for launcher/domain code.
- `jre_lwjgl3glfw` (`JRE`) — Java runtimes and LWJGL packaging.
- `app_pojavlauncher/src/main/jni` and `jniLibs` (`OpenGL`) — GL4ES/Vulkan and native launch bridge.
- `forge_installer` and `arc_dns_injector` — retained upstream helper modules.

The generated package name is `com.elder.launcher`.
...
5. **Build the launcher:** `./gradlew :MainApp:assembleDebug` (Replace `gradlew` with `gradlew.bat` on Windows).
